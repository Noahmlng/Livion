...
CREATE TABLE IF NOT EXISTS users (
  ...
  password TEXT NOT NULL UNIQUE CHECK (password ~ '^[A-Z0-9]{4}-[A-Z0-9]$'), -- 格式"XXXX-X"
...
CREATE POLICY users_policy ON users
  FOR ALL USING (auth.uid()::TEXT = password);

CREATE POLICY goals_policy ON goals
  FOR ALL USING (user_id IN (SELECT user_id FROM users WHERE password = auth.uid()::TEXT));
...
CREATE POLICY task_images_policy ON storage.objects
  FOR ALL USING (
    ...
    (storage.foldername(name))[1] = (SELECT password FROM auth.users WHERE id = auth.uid())
  ); 